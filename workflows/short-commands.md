...
5. **Brief není start** — samotný brief se pouze uloží a potvrdí; AUTO nezačne, dokud UŽIVATEL samostatně nezadá `0`.
...
- Po přijetí briefu EasyTeam nastaví `current_round: 0`, `status: initialization` a čeká na `0`; nesmí předem vytvořit TITLE, Lyrics ani Style Prompt.
...
- EasyTeam nesmí spustit AUTO nebo tvůrčí role pouze po briefu bez samostatného `0` (nebo explicitního `9`).
